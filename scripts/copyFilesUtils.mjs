...
export async function includeFileInBuild(file, target = path.basename(file)) {
  ...
  const targetPath = path.resolve(distPath, target)
...
export async function createPackageFile() {
  ...
  const newPackageData = {
    ...
    ...(packageDataOther.main
      ? {
          main: fse.existsSync(path.resolve(distPath, './node/index.js')) ? './node/index.js' : './index.js',
          module: fse.existsSync(path.resolve(distPath, './esm/index.js')) ? './esm/index.js' : './index.js',
        }
      : {}),
  }
...
export async function prepend(file, str) {
  const data = await fse.readFile(file, 'utf8')
  await fse.writeFile(file, str + data, 'utf8')
}
